...
const deleteOfferByProductId = async (productId: number): Promise<boolean> => {
    try {
        const result: any = await new Promise<any>((resolve, reject) => {
            connection.query('CALL delete_offer_by_product_id(?)', [productId], (error: any, result: any) => {
                if (error) {
...
export default deleteOfferByProductId;